...
class RepsolLuzYGasAPI:
    ...
    def __init__(self, session, username, password):
        ...
        # Initially, house_id and contract_id might not be set
        self.house_id = None
    ...
    # Method to set house_id and contract_id later
    def set_contract_details(self, house_id, contract_id):
                ...
                async with self.session.get(
                    ...
                    if response.status == 200:
                        ...
                        if data:  # Check if data is not empty
                            ...
                            contracts["number_of_contracts"] = len(
                                data.get("contracts", [])
                            )
    ...
    async def async_get_invoices(self):
        ...
        url = INVOICES_URL.format(house_id=self.house_id, contract_id=self.contract_id)
    ...
    async def async_get_costs(self):
        ...
        url = COSTS_URL.format(house_id=self.house_id, contract_id=self.contract_id)
        data = {
            "totalDays": 0,
            "consumption": 0,
            "amount": 0,
    ...
    async def async_get_next_invoice(self):
        """Retrieve cost data for a given contract."""
        headers = {
            "UID": self.uid,
            "signature": self.signature,
        ...
        url = NEXT_INVOICE_URL.format(
            house_id=self.house_id, contract_id=self.contract_id
        )
        data = {
            "amount": 0,
            "amountVariable": 0,
        ...
        try:
            ...
            LOGGER.error("Error fetching next invoice data: %s", e)
        ...
        if "information" in contracts:
            ...
            if len(contracts["information"]) > 0:
                ...
                if invoices:
                    ...
                    data["number_of_contracts"] = len(contracts["information"])
    ...
    async def fetch_all_data(self):
        ...
        try:
            ...
            if not contracts_data:
                ...
                return {"information": []}  # Ensure a consistent structure
